...
func (c *Conn) auth(ctx context.Context) error {
	// Ищем гостевой домен (Prosody требует его для анонимного логина)
	guestDomain := "guest." + c.host
	open := fmt.Sprintf(`<open to="%s" version="1.0" xmlns="urn:ietf:params:xml:ns:xmpp-framing"/>`, guestDomain)
